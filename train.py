...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter out non-image files and depth images
        self.instance_images_path = list(filter(
            lambda path: str(path).lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')) and "_depth" not in path.stem,
        ...
        if class_data_root is not None:
            ...
            # Filter out non-image files and depth images
            self.class_images_path = list(filter(
                lambda path: str(path).lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')) and "_depth" not in path.stem,
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if not instance_depth_image_path.exists():
                raise FileNotFoundError(f"Depth image not found for {instance_image_path}. Please ensure depth images exist.")
            ...
            text_file_path = instance_image_path.with_suffix('.txt')
            ...
            print(f"Skipping corrupted or unidentifiable image: {instance_image_path}")
            # Return a dummy example if the image is corrupted
            dummy_image = torch.zeros((3, self.size, self.size))
            example["instance_images"] = dummy_image
            example["instance_depth_images"] = dummy_image
            example["instance_prompt_ids"] = self.tokenizer(
        ...
        if self.class_data_root:
            ...
            try:
                ...
                if not class_depth_image_path.exists():
                    raise FileNotFoundError(f"Depth image not found for {class_image_path}. Please ensure depth images exist.")
                ...
                print(f"Skipping corrupted or unidentifiable image: {class_image_path}")
                # Return a dummy example if the image is corrupted
                dummy_image = torch.zeros((3, self.size, self.size))
                example["class_images"] = dummy_image
                example["class_depth_images"] = dummy_image
                example["class_prompt_ids"] = self.tokenizer(
                    self.class_prompt,
                    truncation=True,
                    padding="max_length",
...
def create_depth_images(paths, pretrained_model_name_or_path, accelerator, unet, text_encoder):
    ...
    for path in paths:
        ...
        non_depth_image_files = list(filter(lambda path: str(path).lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')) and "_depth." not in str(path), path_iterator))
        ...
        for image_path in tqdm(non_depth_image_files):
            ...
            try:
                ...
                print(f"Skipping corrupted or unidentifiable image: {image_path}")
                continue
